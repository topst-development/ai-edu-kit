...
int app_json_send_results(app_context_t *app)
{
    ...
    for (i = 0; i < app->model_count; ++i) {
        ...
        if (model->post_type != TELECHIPS_NPU_POST_CUSTOM ||
            model->custom_result_kind != APP_CUSTOM_RESULT_LANE) {
            continue;
        }
    ...
    if (append_json(json_buf, sizeof(json_buf), &pos, "],\"hands\":[") < 0) {
        return -1;
    }
    ...
    for (i = 0; i < app->model_count; ++i) {
        ...
        hand_count = model->hand_result.cnt;
        if (hand_count > APP_HAND_MAX_DET) {
            hand_count = APP_HAND_MAX_DET;
        }

        for (int h = 0; h < hand_count; ++h) {
            const hand_pose_obj_t *hand = &model->hand_result.obj[h];
            float x1;
            float y1;
            float x2;
            float y2;
            ...
            if (append_json(json_buf, sizeof(json_buf), &pos,
                            "{\"model\":%d,\"hand_id\":%d,\"score\":%.3f,"
                            "\"x_min\":%.1f,\"y_min\":%.1f,\"x_max\":%.1f,\"y_max\":%.1f,"
                            "\"keypoints\":[",
                            model->index, h, hand->conf,
                            x1, y1, x2, y2) < 0) {
                return -1;
            }
            ...
            for (int k = 0; k < APP_HAND_NUM_KPT; ++k) {
                ...
                if (k > 0 && append_json(json_buf, sizeof(json_buf), &pos, ",") < 0) {
                    return -1;
                }
    ...
    for (i = 0; i < app->model_count; ++i) {
        ...
        seg_count = model->seg_result.cnt;
        if (seg_count > APP_SEG_MAX_DET) {
            seg_count = APP_SEG_MAX_DET;
        }

        for (int s = 0; s < seg_count; ++s) {
            const seg_obj_t *obj = &model->seg_result.obj[s];
            float x1;
            float y1;
            float x2;
            float y2;
            ...
            x1 = obj->x1 * ((float)app->camera_width / model_w);
            y1 = obj->y1 * ((float)app->camera_height / model_h);
            x2 = obj->x2 * ((float)app->camera_width / model_w);
            y2 = obj->y2 * ((float)app->camera_height / model_h);
            if (append_json(json_buf, sizeof(json_buf), &pos,
                            "{\"model\":%d,\"segment_id\":%d,\"cls\":%d,\"score\":%.3f,"
                            "\"x_min\":%.1f,\"y_min\":%.1f,\"x_max\":%.1f,\"y_max\":%.1f,"
                            "\"mask_w\":%d,\"mask_h\":%d}",
                            model->index, s, obj->cls, obj->conf,
                            x1, y1, x2, y2,
                            model->seg_result.mask_w, model->seg_result.mask_h) < 0) {
                return -1;
            }
        }
    }
