...
static void print_fourcc(uint32_t fourcc, char out[5])
{
...
int camera_set_config(camera_handle_t handle, uint32_t width, uint32_t height)
{
    ...
    {
        char fourcc[5];
        print_fourcc(format.fmt.pix_mp.pixelformat, fourcc);
        printf("[camera] negotiated format=%s width=%u height=%u sizeimage=%u bytesperline=%u planes=%u\n",
               fourcc,
                  ...
                  scaler_image_t src, scaler_image_t dst)
{
    ...
    scaler_info.src_winLeft = src.win_left;
    scaler_info.src_winTop = src.win_top;
    scaler_info.src_winRight = src.win_right > src.win_left ? src.win_right : src.width;
    scaler_info.src_winBottom = src.win_bottom > src.win_top ? src.win_bottom : src.height;
    ...
    scaler_info.dest_winLeft = dst.win_left;
    scaler_info.dest_winTop = dst.win_top;
    scaler_info.dest_winRight = dst.win_right > dst.win_left ? dst.win_right : dst.width;
    scaler_info.dest_winBottom = dst.win_bottom > dst.win_top ? dst.win_bottom : dst.height;
