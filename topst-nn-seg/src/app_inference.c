...
static app_custom_result_kind_t detect_custom_result_kind(model_context_t *model)
{
    const char *network_name = NULL;

    if (model == NULL || model->post_type != TELECHIPS_NPU_POST_CUSTOM) {
        return APP_CUSTOM_RESULT_NONE;
    }

    if (model->net != NULL && model->net->methods != NULL &&
        model->net->methods->get_network_name != NULL) {
        network_name = model->net->methods->get_network_name();
    }

    if (contains_case_insensitive(model->path, "face") ||
        contains_case_insensitive(network_name, "face")) {
        return APP_CUSTOM_RESULT_FACE;
    }
    if (contains_case_insensitive(model->path, "hand") ||
        contains_case_insensitive(model->path, "pose") ||
        contains_case_insensitive(network_name, "hand") ||
        contains_case_insensitive(network_name, "pose")) {
        return APP_CUSTOM_RESULT_HAND;
    }
    if (contains_case_insensitive(model->path, "seg") ||
        contains_case_insensitive(network_name, "seg")) {
        return APP_CUSTOM_RESULT_SEG;
    }

    return APP_CUSTOM_RESULT_LANE;
}
...
int init_model(model_context_t *model)
{
    ...
    model->custom_result_kind = detect_custom_result_kind(model);
    ...
    printf("[model%d] cluster=%d name=%s input=%dx%d type=%d custom=%d\n",
           model->index, model->cluster,
           model->net->methods->get_network_name(),
           model->input_width, model->input_height,
           model->post_type, model->custom_result_kind);
...
int postprocess_model(model_context_t *model)
{
    ...
    if (model->post_type == TELECHIPS_NPU_POST_CUSTOM) {
        if (model->custom_result_kind == APP_CUSTOM_RESULT_FACE) {
            memset(&model->det_result, 0, sizeof(model->det_result));
            return network_run_postprocess(model->net, model->output_buf,
                                           &model->det_result);
        }
        if (model->custom_result_kind == APP_CUSTOM_RESULT_HAND) {
            memset(&model->hand_result, 0, sizeof(model->hand_result));
            return network_run_postprocess(model->net, model->output_buf,
                                           &model->hand_result);
        }
        if (model->custom_result_kind == APP_CUSTOM_RESULT_SEG) {
            memset(&model->seg_result, 0, sizeof(model->seg_result));
            return network_run_postprocess(model->net, model->output_buf,
                                           &model->seg_result);
        }
