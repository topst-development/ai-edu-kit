...
int app_config_parse_args(app_context_t *app, int argc, char **argv)
{
    ...
    /* Single-model use cases such as segmentation allow only -n. */
    if (app->models[0].path[0] == '\0') {
        return -1;
    }

    app->model_count = (app->models[1].path[0] == '\0') ? 1 : 2;
    ...
    printf("[input] mode=%s source=%ux%u port=%d json=%s models=%d\n",
