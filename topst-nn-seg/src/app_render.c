...
static void seg_model_to_camera_point(const app_context_t *app,
                                      const model_context_t *model,
                                      float mx,
                                      float my,
                                      float *cx,
                                      float *cy)
{
    const float model_w = (model->input_width > 0) ? (float)model->input_width : (float)APP_SEG_MODEL_WIDTH;
    const float model_h = (model->input_height > 0) ? (float)model->input_height : (float)APP_SEG_MODEL_HEIGHT;

    *cx = mx * ((float)app->camera_width / model_w);
    *cy = my * ((float)app->camera_height / model_h);
}

static void blend_rgb_pixel(uint8_t *base,
                            uint32_t width,
                            uint32_t height,
                            int x,
                            int y,
                            ...
                            uint8_t b)
{
    ...
    if (base == NULL || x < 0 || y < 0 ||
        x >= (int)width || y >= (int)height) {
        return;
    }

    p = &base[((size_t)y * (size_t)width + (size_t)x) * 3u];
    p[0] = (uint8_t)(((int)p[0] * 3 + (int)r * 2) / 5);
    p[1] = (uint8_t)(((int)p[1] * 3 + (int)g * 2) / 5);
    p[2] = (uint8_t)(((int)p[2] * 3 + (int)b * 2) / 5);
}

static void draw_seg_results(const app_context_t *app,
                             const model_context_t *model,
                             uint8_t *output_map_base,
                             Color_t color,
                             double box_font_size,
                             int box_label_offset)
{
    ...
    const seg_objs_t *seg = &model->seg_result;
    int count = seg->cnt;
    int mask_w = seg->mask_w > 0 ? seg->mask_w : APP_SEG_MASK_WIDTH;
    int mask_h = seg->mask_h > 0 ? seg->mask_h : APP_SEG_MASK_HEIGHT;

    if (count > APP_SEG_MAX_DET) {
        count = APP_SEG_MAX_DET;
    }

    for (int i = 0; i < count; ++i) {
        const seg_obj_t *obj = &seg->obj[i];
        const uint8_t *rgb = palette[i % (int)(sizeof(palette) / sizeof(palette[0]))];
        Box_t box;
        float cx1;
        float cy1;
        float cx2;
        ...
        for (int my = 0; my < mask_h && my < APP_SEG_MASK_HEIGHT; my += 2) {
            for (int mx = 0; mx < mask_w && mx < APP_SEG_MASK_WIDTH; mx += 2) {
                int x0;
                int x1;
                int y0;
                int y1;

                if (obj->mask[my][mx] == 0u) {
                    continue;
                }

                x0 = (mx * (int)app->display_width) / mask_w;
                x1 = ((mx + 2) * (int)app->display_width) / mask_w;
                y0 = (my * (int)app->display_height) / mask_h;
                y1 = ((my + 2) * (int)app->display_height) / mask_h;
                if (x1 <= x0) {
                    x1 = x0 + 1;
                }
                if (y1 <= y0) {
                ...
                if (x1 > (int)app->display_width) { x1 = (int)app->display_width; }
                if (y1 > (int)app->display_height) { y1 = (int)app->display_height; }
                for (int py = y0; py < y1; ++py) {
                    for (int px = x0; px < x1; ++px) {
                        blend_rgb_pixel(output_map_base, app->display_width, app->display_height,
        ...
        seg_model_to_camera_point(app, model, obj->x1, obj->y1, &cx1, &cy1);
        seg_model_to_camera_point(app, model, obj->x2, obj->y2, &cx2, &cy2);
        box.cls = obj->cls;
        box.track_id = i;
        box.score = obj->conf;
        box.xmin = (int)(cx1 + 0.5f);
        box.ymin = (int)(cy1 + 0.5f);
        box.xmax = (int)(cx2 + 0.5f);
        box.ymax = (int)(cy2 + 0.5f);
        cvDrawBoxes(output_map_base, &box, 1,
                    app->display_width, app->display_height,
                    app->camera_width, app->camera_height,
                    color, color, box_font_size, box_label_offset);
    ...
    for (i = 0; i < app->model_count; ++i) {
        ...
        if (model->post_type == TELECHIPS_NPU_POST_DETECTOR ||
            ...
            draw_hand_pose_results(app, model, output_map_base,
                   ...
                   model->custom_result_kind == APP_CUSTOM_RESULT_SEG) {
            draw_seg_results(app, model, output_map_base,
                             color, box_font_size, box_label_offset);
        } else if (model->post_type == TELECHIPS_NPU_POST_CUSTOM) {
